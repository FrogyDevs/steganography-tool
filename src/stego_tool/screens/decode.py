...
class DecodeScreen(Screen):
    def compose(self):
        yield Static("Decode Screen")
        yield Label("Selected file: ", id="result-label")
        yield Label('Message in file: ', id='msg')
    ...
    def on_file_selected(self, selected: FileSelected | None) -> None:
        ...
        try:
            result = codec.decode()
            self.query_one('#msg', Label).update(f'Message in file: {result}')
        except:
            self.query_one('#msg', Label).update(f'No message in file')
            return
